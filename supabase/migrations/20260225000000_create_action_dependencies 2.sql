...
CREATE TABLE action_dependencies (
  ...
  blocker_action_id UUID NOT NULL REFERENCES actions(id) ON DELETE CASCADE,
  created_at TIMESTAMPTZ DEFAULT NOW(),
  UNIQUE(blocked_action_id, blocker_action_id),
  CHECK (blocked_action_id != blocker_action_id)
);

CREATE INDEX idx_action_dependencies_blocked ON action_dependencies(blocked_action_id);
CREATE INDEX idx_action_dependencies_blocker ON action_dependencies(blocker_action_id);
